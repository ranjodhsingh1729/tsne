...
MatType binarySearchPerplexity(const MatType &X, double perplexity)
{
  ...
  MatType D = mlpack::PairwiseDistances(X, mlpack::SquaredEuclideanDistance());
...
class TSNE
{
 public:
      ...
      : dimensions(dimensions), perplexity(perplexity)
  {
    ...
    P = binarySearchPerplexity(X, perplexity);
  ...
  {
    MatType Q;
    Q = mlpack::PairwiseDistances(y, mlpack::SquaredEuclideanDistance());
    Q = 1.0 / (1.0 + Q);
    Q.diag().zeros();
    Q /= arma::accu(Q);
  ...
  {
    MatType q_unnorm, Q;
    q_unnorm = mlpack::PairwiseDistances(Y, mlpack::SquaredEuclideanDistance());
    q_unnorm = 1.0 / (1.0 + q_unnorm);
    q_unnorm.diag().zeros();

    Q = q_unnorm / std::max(arma::accu(q_unnorm), arma::datum::eps);
    Q.elem(arma::find(Q < arma::datum::eps)).fill(arma::datum::eps);

    MatType M = (P - Q) % q_unnorm;
                              ...
                              const size_t n)
  {
    ...
    MatType Q = q_unnorm / std::max(arma::accu(q_unnorm), arma::datum::eps);
    Q.elem(arma::find(Q < arma::datum::eps)).fill(arma::datum::eps);
    ...
    g = 4.0 * (y * arma::diagmat(S) - y * M);
    return n * arma::accu(P % arma::log(P / Q));
  }
...
int main(int argc, char **argv)
{
  ...
  ens::MomentumSGD optimizer1(
      500.0, X.n_cols, 20 * X.n_cols, 1e-5, false, ens::MomentumUpdate(0.5));
  ens::MomentumSGD optimizer2(
      500.0, X.n_cols, 80 * X.n_cols, 1e-5, false, ens::MomentumUpdate(0.8));
  ens::MomentumSGD optimizer3(
      500.0, X.n_cols, 220 * X.n_cols, 1e-5, false, ens::MomentumUpdate(0.8));
